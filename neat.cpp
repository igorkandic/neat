...
//}
//
int NEAT::getUnitCount(std::string string, std::string set)
{
	int count = 0;
	short flag = 0;

	for(auto last : string)
	{
		flag = last;
...
bool NEAT::load_neat_params(std::string filename, bool output) {
	...
	// **********LOAD IN PARAMETERS*************** //
    if(output)
		std::cout << "NEAT READING IN" << filename;
